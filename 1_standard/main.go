...
import (
	...
	"mime"
	"net/http"
	"rest-api-implementations/1_standard/pkg"
	"strconv"
	"strings"
	"time"
)
...
func (ts *taskServer) taskHandler(w http.ResponseWriter, req *http.Request) {
	if req.URL.Path == "/task/" {
		if req.Method == http.MethodPost {
			ts.createTasksHandler(w, req)
		} else if req.Method == http.MethodGet {
			ts.getAllTasksHandler(w, req)
		} else if req.Method == http.MethodDelete {
			ts.deleteAllTasksHandler(req)
		} else {
...
func (ts *taskServer) createTasksHandler(w http.ResponseWriter, req *http.Request) {
...
func (ts *taskServer) deleteAllTasksHandler(req *http.Request) {
